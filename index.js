...
import { BASE_GITIGNORE, NEXTJS_GITIGNORE, VITE_GITIGNORE, } from "./gitignore.js";
import { intro, confirm, select, multiselect, isCancel, cancel, text, } from "@clack/prompts";
 ...
 */
async function createYarnRcYml(baseDir, chalk) {
...
`;
    ...
    try {
        await fs.promises.writeFile(path.join(baseDir, fileName), yarnRcYmlContents);
    }
    catch (_) {
 ...
 * @param {ChalkInstance} chalk - The chalk instance for logging
 */
async function setPackageJsonFields(packageManager, baseDir, chalk) {
    ...
    try {
        ...
        console.log(`${chalk.red("Error: ")} Failed to set package manager in package.json`);
    }
}
/**
 ...
 * @param {ChalkInstance} chalk - The chalk instance for logging
 */
async function writeGitignore(baseDir, framework, chalk) {
    try {
        ...
        const frameworkGitignorePath = path.join(baseDir, "apps", "web", ".gitignore");
        if (framework === "nextjs") {
 ...
 */
async function updateLangGraphConfig(baseDir, chalk, args) {
    try {
        const langGraphConfigPath = path.join(baseDir, "langgraph.json");
        const config = JSON.parse(await fs.promises.readFile(langGraphConfigPath, "utf8"));
        ...
        await fs.promises.writeFile(langGraphConfigPath, JSON.stringify(config, null, 2) + "\n");
    }
    catch (_) {
 ...
 */
const createStartServersMessage = (chalk, packageManager, framework) => {
 ...
 */
async function setAgentPackageJsonFields(baseDir, inputs) {
    const { agentArgs, packageManager, chalk } = inputs;
    try {
        const agentsPkgJsonPath = path.join(baseDir, "apps", "agents", "package.json");
        const pkgJson = JSON.parse(await fs.promises.readFile(agentsPkgJsonPath, "utf8"));
        ...
        if (agentArgs.includeRetrievalAgent) {
            Object.assign(requiredPackages, AGENT_DEPENDENCIES_MAP["retrieval-agent"]);
        }
        ...
        pkgJson.scripts["build:internal"] = pkgJson.scripts["build:internal"].replace("{PACKAGE_MANAGER}", packageManager);
        await fs.promises.writeFile(agentsPkgJsonPath, JSON.stringify(pkgJson, null, 2));
    }
    catch (_) {
        console.log(`${chalk.red("Error: ")} Failed to set agent package.json fields`);
    }
}
 ...
 * @param {ChalkInstance} chalk - The chalk instance for logging
 */
async function setEnvExampleFile(baseDir, args, chalk) {
    try {
        const envExamplePath = path.join(baseDir, ".env.example");
        const requiredEnvVarsSet = new Set();
        if (args.includeReactAgent) {
            AGENT_ENV_VARS_MAP["react-agent"].forEach((v) => requiredEnvVarsSet.add(v));
        }
        if (args.includeMemoryAgent) {
            AGENT_ENV_VARS_MAP["memory-agent"].forEach((v) => requiredEnvVarsSet.add(v));
        }
        if (args.includeResearchAgent) {
            AGENT_ENV_VARS_MAP["research-agent"].forEach((v) => requiredEnvVarsSet.add(v));
        }
        if (args.includeRetrievalAgent) {
            AGENT_ENV_VARS_MAP["retrieval-agent"].forEach((v) => requiredEnvVarsSet.add(v));
        }
 ...
 * @param {ChalkInstance} chalk - The chalk instance for logging
 */
async function createPnpmWorkspacesFile(baseDir, chalk) {
    try {
...
`;
        ...
        console.log(`${chalk.red("Error: ")} Failed to create pnpm workspaces file`);
    }
}
/**
 ...
 * @param {ChalkInstance} chalk - The chalk instance for logging
 */
async function addPnpmDirectDependencyWorkaround(baseDir, chalk) {
    try {
        const agentsPkgJsonPath = path.join(baseDir, "apps", "agents", "package.json");
        const pkgJson = JSON.parse(await fs.promises.readFile(agentsPkgJsonPath, "utf8"));
        ...
        console.log(`${chalk.red("Error: ")} Failed to update agents package.json dependencies`);
    }
}
/**
...
async function parseCommandLineArgs() {
    ...
    program
        ...
        .option("--project-name <name>", "Name of the project (default: agent-chat-app)")
        .option("--package-manager <manager>", "Package manager to use (npm, pnpm, yarn) (default: npm)")
        .option("--install-deps <boolean>", "Automatically install dependencies (default: true)")
        .option("--framework <framework>", "Framework to use (nextjs, vite) (default: vite)")
        .option("--include-agent <agent...>", "Pre-built agents to include (react, memory, research, retrieval) (default: all)")
        .allowUnknownOption();
    program.parse();
 ...
 */
async function promptUser(partialAnswers = {}) {
    ...
    if (!packageManager) {
        ...
        packageManager = packageManagerResponse;
    }
    ...
    if (autoInstallDeps === undefined) {
        ...
        autoInstallDeps = autoInstallDepsResponse;
    }
    // Framework prompt
    ...
    const allAgentSelectionsProvided = partialAnswers.includeReactAgent !== undefined &&
    ...
    if (!allAgentSelectionsProvided) {
        const selectedAgentsResponse = await multiselect({
            message: 'Which pre-built agents would you like to include? (Press "space" to select/unselect)',
            options: [
        ...
        const selectedAgents = selectedAgentsResponse;
...
async function init() {
    ...
    const allRequiredOptionsProvided = cliOptions.autoInstallDeps !== undefined &&
    ...
    const monorepoTemplateDir = path.join(__dirname, "templates", "monorepo");
    ...
    const frameworkTemplateDir = path.join(__dirname, "templates", framework);
    ...
    const agentsDir = path.join(appsDir, "agents", "src");
    ...
    const rootPkgJsonPath = path.join(targetDir, "package.json");
    if (await fs.exists(rootPkgJsonPath)) {
        const rootPkgJson = JSON.parse(await fs.readFile(rootPkgJsonPath, "utf8"));
...
async function copyAgentTemplate(agentName, agentsDir) {
    const agentTemplateDir = path.join(__dirname, "templates", agentName);
    // Determine the destination directory for the agent
    const agentDestDir = path.join(agentsDir, agentName);
